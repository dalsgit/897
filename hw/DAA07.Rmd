...
The lowest CV is for 49 components. This amounts to simply performing least squares, because when all of the components are used in PCR no dimension reduction occurs. 
However, from the plot we also see that the cross-validation error is roughly the same when only 30
components are included in the model. This suggests that a model that uses just a small number of components might suffice.
...
The % variance explained of parameters with all the components (49) = 96.69
The % variance explained of response with all the components (49) = 99.52

Let's also get the MSE with 30 components
```{r}
pcr.pred=predict (pcr.fit, xtest, ncomp =30)
mean((pcr.pred - ytest)^2)
```
The MSE with 30 components = 0.120898
The % variance explained of parameters with 30 components = 87.72
The % variance explained of response with 30 components = 98.04
...
The lowest CV is for 9 components after which it only increases slightly and stays constant after 22 components before increasing drastically at 49 components. 
...
```{r}
pls.fit=plsr(y~., data=combined.data, scale=TRUE, validation="CV", segments=5)
summary (pls.fit )
```
...
The test MSE with ridge is: 0.1508586
...
The test MSE with lasso is: 0.1982305
